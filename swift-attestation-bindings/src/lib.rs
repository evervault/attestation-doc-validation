...
impl PCRProvider for &PCRs {
  fn pcr_0(&self) -> Option<&str> {
    unsafe { (*self).pcr_0.as_ref().and_then(|c_str| CStr::from_ptr(c_str).to_str().ok()) }
  }

  fn pcr_1(&self) -> Option<&str> {
    unsafe { (*self).pcr_1.as_ref().and_then(|c_str| CStr::from_ptr(c_str).to_str().ok()) }
  }

  fn pcr_2(&self) -> Option<&str> {
    unsafe { (*self).pcr_2.as_ref().and_then(|c_str| CStr::from_ptr(c_str).to_str().ok()) }
  }

  fn pcr_8(&self) -> Option<&str> {
    unsafe { (*self).pcr_8.as_ref().and_then(|c_str| CStr::from_ptr(c_str).to_str().ok()) }
  }
}
