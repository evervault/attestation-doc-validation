...
pub fn validate_attestation_document_structure(
    ...
    let valid_structure_check = !attestation_document.module_id.is_empty()
        ...
        && attestation_document
            .public_key
            .as_ref()
            .map(|key| key.len() > 0 && key.len() <= 1024)
            .unwrap_or(true) // these default to true if not present
        && attestation_document
            .nonce
            .as_ref()
            .map(|nonce| nonce.len() > 0 && nonce.len() <= 512)
            .unwrap_or(true)
        && attestation_document
            .user_data
            .as_ref()
            .map(|user_data| user_data.len() > 0 && user_data.len() <= 512)
            .unwrap_or(true);
...
fn pcrs_match(attestation_doc: &AttestationDoc, expected_pcrs: PCRs) -> Result<()> {
    ...
    let same_pcrs = Some(&expected_pcrs.pcr_0) == encoded_measurements.get(&0)
        && Some(&expected_pcrs.pcr_1) == encoded_measurements.get(&1)
        && Some(&expected_pcrs.pcr_2) == encoded_measurements.get(&2)
        && Some(&expected_pcrs.pcr_8) == encoded_measurements.get(&8);
    true_or_invalid(same_pcrs, AttestationError::UnexpectedPCRs)
}

pub fn validate_attestation_doc(
    attestation_doc_cose_sign_1_bytes: Vec<u8>,
    expected_pcrs: PCRs,
) -> Result<AttestationDoc> {
    let cose_sign_1_decoded: CoseSign1 =
        serde_cbor::from_slice(&attestation_doc_cose_sign_1_bytes)?;
...
mod attestation_tests {
    ...
    fn validate_valid_attestation_doc_unexpected_pcrs_time_sensitive() {
        ...
        assert!(matches!(err, AttestationError::UnexpectedPCRs));
    }
